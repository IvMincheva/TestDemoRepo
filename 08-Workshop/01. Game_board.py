...
def get_down_right_win_condition_values(board, row_index, column_index, max_row_index, max_column_index):
    # values = [board[row_index + d][column_index + d] for d in range(max_row_index - column_index)]
    # return values
    values = []
    max_d = min(max_row_index - row_index, max_column_index - column_index)
...
def get_down_left_win_condition_values(board, row_index, column_index, max_row_index, min_column_index):
    ...
    # return values
    values = []
    max_d = min(max_row_index - row_index, abs(min_column_index - column_index))
    for d in range(max_d):
        r = row_index + d
        c = column_index - d
        values.append(board[r][c])
...
def get_up_right_win_condition_values(board, row_index, column_index, min_row_index, max_column_index):
    values = []
    max_d = min(abs(min_row_index - row_index), max_column_index - column_index)
    for d in range(max_d):
        r = row_index - d
        c = column_index + d
        values.append(board[r][c])
...
def get_up_left_win_condition_values(board, row_index, column_index, min_row_index, min_column_index):
    values = []
    max_d = min(abs(min_row_index - row_index), abs(min_column_index - column_index))
    for d in range(max_d):
        r = row_index - d
        c = column_index - d
        values.append(board[r][c])
...
def check_win_condition(board, row_index, column_index, win_count):
    ...
    up_win_condition_values = get_up_win_condition_values(board, row_index, column_index, min_row_index)
    down_win_condition_values = get_down_win_condition_values(board, row_index, column_index, max_row_index)

    up_left_win_condition_values = get_up_left_win_condition_values(board, row_index, column_index, min_row_index, min_column_index)
    up_right_win_condition_values = get_up_right_win_condition_values(board, row_index, column_index, min_row_index, max_column_index)
    down_left_win_condition_values = get_down_left_win_condition_values(board, row_index, column_index, max_row_index, min_column_index)
    down_right_win_condition_values = get_down_right_win_condition_values(board, row_index, column_index, max_row_index, max_column_index)
    ...
    # return len(up_win_condition_values) == win_count and len(set(up_win_condition_values)) == 1
    # return any(
...
def is_player_choice_valid(board, player_choice):
    ...
    has_column_space = is_column_index_valid and board[0][player_choice] is None
